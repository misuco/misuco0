...
//		int resolution=m.h;
		...
		int div=300*heartbeat;
		int divy=300*heartbeat;
		int prevon=-1;
		int prevony=-1;
...
//			m.seqgrid.pointer+j;
			...
			// adjust the start, so that the latest is drawed last..
			if(timef<0.1f) {
				lastStartK=k;
			}
...
//				int xseq=m.w-(j*m.w/m.seqgrid.len);
				...
				* (int) ((systime - m.seqgrid.t[k]) % div)
				/ div;
				int yseq = m.h
				* (int) ((systime - m.seqgrid.t[k]) % divy)
				/ divy;
...
//					}
					...
					if(j==0 && m.seqgrid.ts[k].type==MisuSignal.NOTE_OFF) {
						...
						prevon= m.w
						* (int) ((systime - m.seqgrid.t[kprev]) % div)
						/ div;
						prevony= m.h
						* (int) ((systime - m.seqgrid.t[kprev]) % div)
						/ div;
